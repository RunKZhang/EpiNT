...
https://github.com/PandasSquad/CHB-MIT_Seizure_Prediction/blob/main/src/data_handler.py
"""
...
# config = {
#     "CHB_FOLDER_DIR": DATASETS_PATH["CHB_MIT"],
# }

class Handler:
    def __init__(self, path: str):
        """
        Initialize the handler with the path to the CHB-MIT dataset folder.
        
        :param path: Path to the CHB-MIT dataset folder
        """
        self.config = {
            "CHB_FOLDER_DIR": path,
        }
    ...
    def get_patients(self) -> List[str]:
        ...
        """
        path = self.config["CHB_FOLDER_DIR"]
    ...
    def get_edf_data(self, patient: str, edf: str, new_sfreq: int) -> pd.DataFrame:
        ...
        # resample the data
        if new_sfreq > mne_data.info["sfreq"]:
            mne_data.resample(new_sfreq)
        return mne_data.to_data_frame()


    def get_seizure_times(self, patient: str) -> List[Tuple[int, int]]:
        ...
        """
        summary = self.get_summary(patient)
        seizure_start_times = [
        ...
        number_of_seizures = self.get_number_of_seizures(patient)
    ...
    def get_number_of_seizures(self, patient: str) -> Dict[str, int]:
    ...
    def get_seizure_data(self, patient: str) -> pd.DataFrame:
        """Get the seizure data for a patient

        :param patient: The patient name
        :return: A dataframe with the file names, number of seizures, start and end times
        """
        file_names_and_seizures = self.get_number_of_seizures(patient)
        seizure_times = self.get_seizure_times(patient)
        df = pd.DataFrame(
            {
    ...
    def get_seizure_time_from_edf(self, patient: str, edf: str) -> List[Tuple[int, int]]:
        ...
        df = self.get_seizure_data(patient)
    ...
    def make_patient_windows(self, patient: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        seizures = self.get_seizure_data(patient)
        ...
        for edf_file, seizure_time in zip(edf_files_with_seizures, seizure_times):
            edf_data = self.get_edf_data(patient, edf_file)
